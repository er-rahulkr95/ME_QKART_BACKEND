...
const getUser = catchAsync(async (req, res) => {
      ...
      const getUser = await userService.getUserById(userIdParam);
      if(!getUser){
        throw new ApiError(httpStatus.NOT_FOUND, "User not found")
      }
      if(getUser.email !== req.user.email){
          throw new ApiError(httpStatus.FORBIDDEN, "Not authorized to get details")
      }

      if(req.query.q === "address"){
        const userAddress =  await userService.getUserAddressById(userIdParam);
        res.status(httpStatus.OK).json({address:userAddress.address});
      }else{
