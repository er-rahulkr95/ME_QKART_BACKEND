...
const getCartByUser = async (user) => {
    ...
    if(!cart){
      throw new ApiError(httpStatus.NOT_FOUND, "User does not have any item in cart")
    }
    return cart
};
...
const addProductToCart = async (user, productId, quantity) => {
  ...
  if(!cart){
    try {
      ...
      throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, "Can not create user cart")
    }
  }

  const isProductInCart = cart.cartItems.some(item=> item.product._id == productId)
  if(isProductInCart){
...
const checkout = async (user) => {
      ...
      if(!isAddressSet){
        ...
        cartItems.forEach(item => {
          totalMoney = item.product.cost * item.quantity;
        });

      if(user.walletMoney < totalMoney ){
        throw new ApiError(httpStatus.BAD_REQUEST, "Insufficient Balance, Wallet money is low")
      }

      const remainingBalance = user.walletMoney - totalMoney;
      user.walletMoney = remainingBalance;
      userCart.cartItems = []
      await userCart.save()
      await user.save()
      return user;
