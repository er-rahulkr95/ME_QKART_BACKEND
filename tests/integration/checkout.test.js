...
describe("Cart routes", () => {
  describe("Checkout", () => {
    ...
    it("should return 204 if cart is valid", async () => {
      ...
      const cart = await request(app).get(`/v1/cart`).set("Authorization", `Bearer ${userOneAccessToken}`).send();
      expect(cart.status).toEqual(httpStatus.OK)
      expect(JSON.parse(cart.text).cartItems.length).toEqual(0);
    });
  });
});
